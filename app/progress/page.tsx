...
export default function ProgressPage() {
    ...
    // Mock data for demonstration if no active courses are found
        ...
        >
            ...
            <footer className="fixed bottom-0 left-0 right-0 border-t-[3px] border-border px-6 py-4 lg:px-40 bg-surface z-40 flex items-center justify-between">
                <div className="flex items-center gap-6 text-sm text-muted">
                    <span className={`flex items-center gap-2 bg-mint/20 px-3 py-1 rounded-full border-2 border-border ${isActive ? 'animate-gentle-pulse' : ''}`}>
                ...
                <div className="hidden sm:block">
                    <p className="text-xs text-muted font-semibold">Total Activity: {displayProgress.length} Course(s) Live</p>
                </div>
            </footer>
        </AppShell>
    );
